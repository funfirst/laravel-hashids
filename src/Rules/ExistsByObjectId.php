...
namespace FF\LaravelHashids\Rules;
...
class ExistsByObjectId extends Exists implements ValidationRule, ValidatorAwareRule
{
    ...
    /**
     * @param class-string<Model&HasHashid> $class
     */
    public function __construct(string $class)
    {
        $this->model = new $class();

        if (!method_exists($this->model, 'bootHasHashid')) {
            throw new InvalidArgumentException('Class does not use HashId trait');
        }
